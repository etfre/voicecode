...
class SubprocessEngine(BaseEngine):
    def __init__(self, process_cmd, filter_func=lambda x: x):
        self.p = subprocess.Popen(process_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        self.filter_func = filter_func

    def get_lines(self):
        for line in self.p.stdout:
            line = self.filter_func(line.decode('utf8')) if line is not None else line
            if line:
                yield line
